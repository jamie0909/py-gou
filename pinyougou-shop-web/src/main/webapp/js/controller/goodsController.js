...
app.controller('goodsController', function ($scope, $controller, $location, typeTemplateService, itemCatService, uploadService, goodsService) {
    ...
    //'0'       '1'
    $scope.isMarketableStatus = ['再次上架', '提交下架']; //定义一个商品上下架状态

    //查询上下架商品 by whk
    $scope.findIsMarketable = function (isMarketableStatus) {
        goodsService.search($scope.paginationConf.currentPage, $scope.paginationConf.itemsPerPage, $scope.searchEntity = {isMarketable: isMarketableStatus}).success(
            function (response) {
                $scope.list = response.rows;
                $scope.paginationConf.totalItems = response.total;//更新总记录数
            }
    ...
    //根据商品id,更改商品上下架状态 by whk
    $scope.updateIsMarketableStatus = function (id, isMarketableStatus) {
        goodsService.updateIsMarketableStatus(id, isMarketableStatus).success(
            function (response) {
                if (response.success) {
                    $scope.reloadList();//刷新列表
                } else {
