...
@Service
@Transactional
public class OrderServiceImplHu implements OrderServiceHu {
	...
	public Map add(TbOrder order) {
		...
		if("1".equals(order.getPaymentType())){
			...
			System.out.println("==创建payLog，放入数据库和缓存===========");
			System.out.println(order.getOrderId());
			redisTemplate.boundHashOps("payLog").put(payLog.getOutTradeNo(), payLog);//放入缓存
		}
	...
	@Override
	public TbPayLog searchPayLogFromRedis(String out_trade_no) {
		System.out.println();
	...
	@Override
	public void updateStatus(String out_order_no, String status) throws Exception {
		TbPayLogExample example = new TbPayLogExample();
		TbPayLogExample.Criteria criteria = example.createCriteria();
		criteria.andOutTradeNoEqualTo(out_order_no);
		List<TbPayLog> tbPayLogs = payLogMapper.selectByExample(example);

        String status1 = tbPayLogs.get(0).getStatus();
        if ("8".equals(status)){
            if ("1".equals(status1)||"6".equals(status1)){
                throw new Exception();
            }
        }
		...
		payLogMapper.updateByExampleSelective(payLog,example);

		String orderListStr = tbPayLogs.get(0).getOrderList();
